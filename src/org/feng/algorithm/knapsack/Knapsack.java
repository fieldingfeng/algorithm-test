		...
		for (int item = 1; item <= N; item++) {
			...
			for (int weight = 1; weight <= W; weight++) {
				...
				if (wt[item - 1] <= weight) {
					...
					int currentItemValue = val[item - 1];
					int maxValueOfRestWeight = V[item - 1][weight - wt[item - 1]];
					
					int previousMaxValue = V[item - 1][weight];
					V[item][weight] = Math.max(currentItemValue + maxValueOfRestWeight, previousMaxValue);
				}
				else {
